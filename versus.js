	...
	return {
		...
		start:function(){
			...
			if(_status.connectMode){
				...
				if(lib.storage.choice==undefined) game.save('choice',10);
				if(lib.storage.zhu==undefined) game.save('zhu',false);
				if(lib.storage.noreplace_end==undefined) game.save('noreplace_end',false);
				if(get.config('first_less')==undefined) game.saveConfig('first_less',false,false);
				if(lib.storage.autoreplaceinnerhtml==undefined) game.save('autoreplaceinnerhtml',false);
				if(lib.storage.single_control==undefined) game.save('single_control',false);
				if(lib.storage.number==undefined) game.save('number',3);
				if(lib.storage.versus_reward==undefined) game.save('versus_reward',0);
				if(lib.storage.versus_punish==undefined) game.save('versus_punish','无');
				if(lib.storage.replace_number==undefined) game.save('replace_number',0);
				...
				switch(lib.storage.seat_order){
					...
					default:lib.storage.cross_seat=true;lib.storage.random_seat=false;
				}
			...
			chooseCharacter:function(){
				...
				next.setContent(function(){
					...
					event.check=function(){
						...
						this.dialog.versus_seat_order=this.dialog.add(ui.create.switcher('seat_order',['交叉','对阵','随机'],lib.storage.seat_order)).querySelector('.toggle');
						...
						this.dialog.choice=this.dialog.add(ui.create.switcher('choice',[10,16,20,24,40,'∞'],lib.storage.choice)).querySelector('.toggle');
					...
					event.custom.add.window=function(){
						...
						switch(lib.storage.seat_order){
							case '交叉':lib.storage.cross_seat=true;lib.storage.random_seat=false;break;
							case '随机':lib.storage.cross_seat=false;lib.storage.random_seat=true;break;
							case '对阵':lib.storage.cross_seat=false;lib.storage.random_seat=false;break;
							default:lib.storage.cross_seat=true;lib.storage.random_seat=false;
						}
